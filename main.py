...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    TASK: Write a product description that is tailored into this customer's Age group and skin type. Use age group specific slang.;
...
prompt = PromptTemplate(
    input_variables=["agegroup", "skintype", "content"],
    template=template,
)
...
def get_hobby():
    input_text = st.text_input(label="Customers skin type", key="hobby_input")
    return input_text
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, skintype=hobby_input, content=content_input)
